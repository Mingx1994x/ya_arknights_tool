...
- 安裝 Vitest 並新增 `pnpm test`／`pnpm test:watch` script；建立獨立的 `tests/` 測試目錄（依測試類型分子目錄，不與原始碼混放，見 `docs/TESTING.md`），並新增 `tests/unit/mastery.test.ts` 對照現行相加模型公式（`docs/domain/arknights_tools_init.md` 第 3、5、7 節）與目前的 `app/utils/mastery.ts` 匯出介面撰寫單元測試，涵蓋 `RequiredWorkBase`／跨階段減半／`phase.work` 換算、`planCriticalCompanionStage` 的 critical＋陪練幹員配對、`suggestStagePlans` 的自動排程反推。
