...
- Tab B（`ManualPlanTab.vue`）採用**預設策略**：專精一、二各安排一位 critical 幹員（Logos／艾麗妮）陪同（下限 5hr、預設 5hr5min 含操作緩衝），觸發下一階段減半後，反推「另一位陪練幹員」需要陪同多久才能補滿所需工作量；專精三沒有下一階段可減半，因此不安排 critical 幹員，直接反推單一陪練幹員的所需時長。因為專精一、二一定會安排 critical 幹員，`RequiredWork(2)`／`RequiredWork(3)` 在此策略下必定套用減半，不需要額外輸入「上一階段是否觸發減半」。
- Tab A（`AutoPlanTab.vue`）採用跟 Tab B 概念上相同的**預設策略**（critical 幹員＋另一位陪練幹員），差別是候選幹員不用手動選：專精一、二自動挑該階段 `critical` 類別效率最高的當 critical 幹員（固定陪同 `CRITICAL_DEFAULT_DURATION_HOURS`）、非 `critical` 類別效率最高的當另一位陪練幹員；專精三自動挑整體效率最高的候選幹員。`suggestStagePlans` 把使用者選擇的起始階段（`phases[0]`）視為宣告式的假設起點，永遠當作未減半，之後的階段依序視為套用本策略而觸發減半——這一版已跟 Tab B 分岔（Tab B 仍是固定假設專精一、二一定安排 critical 幹員，見下方「刻意尚未實作」）。

刻意**尚未實作**（下一階段工作）：
- **`ManualPlanTab.vue` 尚未套用「相對起始階段」的減半假設**：`AutoPlanTab.vue` 已改成把使用者選擇的起始階段視為未減半、之後階段依序觸發減半（見上方 Tab A 說明）；`ManualPlanTab.vue` 仍是舊版邏輯（`getRequiredWorkUnderDefaultStrategy` 寫死「專精一未減半、專精二／三一定減半」，不管起始階段選哪裡），也還沒有 Tab A 那種「一次列出起始階段→專精三所有階段」的排版。兩個分頁的設計目前已分岔，重新設計 `ManualPlanTab.vue` 時要一併補上，見 [domain 文件第 9 節](./domain/arknights_tools_init.md#9-尚未收斂的部分)。
